...
class DeepseekDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        start_att = torch.cuda.Event(enable_timing=True)
        end_att = torch.cuda.Event(enable_timing=True)                
        start_att.record()    
        # Self Attention
        ...
        elapsed_att = start_att.elapsed_time(end_att)
        ...
        start_moe = torch.cuda.Event(enable_timing=True)
        end_moe = torch.cuda.Event(enable_timing=True)
        start_moe.record()
        hidden_states = self.mlp(hidden_states)
        end_moe.record()
        torch.cuda.synchronize()
        elapsed_moe = start_moe.elapsed_time(end_moe)        

        # print("att={:.2f}, moe={:.2f}".format(elapsed_att, elapsed_moe))
            # 文件名
        filename = "measure_data_deepseek/tmp_data.txt"
        # 检查文件是否存在
        if os.path.exists(filename):
            with open(filename, "a") as f:
                f.write("att={:.2f}, moe={:.2f}\n".format(elapsed_att, elapsed_moe))                    
